// ═══════════════════════════════════════════════════════════════════════════
// OmniLink - LocalCableBook Component
// Carnet de câbles LOCAL TECHNIQUE : BN bornier image → Automate
// ═══════════════════════════════════════════════════════════════════════════
...
export function LocalCableBook({ localRecords, cableTypes = [], onUpdate, onDelete, onBulkAdd }) {
  ...
  const handleBulkAdd = async () => {
    ...
    try {
      ...
      notify(`${count} instrument(s) ajouté(s) au carnet Local Technique`);
  ...
  const handleExport = async () => {
    try {
      const workbook = await CableBookExportService.exportLocal(localRecords);
      await CableBookExportService.downloadExcel(workbook, 'Carnet_LocalTech_Artelia');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border-2 border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-800">🏢 Carnet Local Technique — BN → Automate</h3>
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="bg-[#00375A] text-white">
                ...
                <th className="px-4 py-3 text-left font-semibold">BN TAG</th>
          ...
          <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 flex items-center justify-between text-sm">
            <span className="text-gray-600">
              <strong>{localRecords.length}</strong> instrument(s)
            </span>
            <span className="text-gray-600">
              <strong>{localRecords.filter(r => r.cable_type && r.terminal).length}</strong> complet(s)
            </span>
          </div>
        </div>
      )}
